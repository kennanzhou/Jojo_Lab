...
- Word Camp settings should include a clear way to reselect only today's word plan. This must reset `dailyWordPlan` and immediately persist it, without clearing word mastery, word banks, Word Camp small stars, or global big stars.
...
- Card Cottage's title area should stay compact: show the real global big gold star as an icon with its count, plus a single revealed/total counter such as "0/100". Do not show label chips like "大金星", "已翻开", or "待翻开" in the top area.
- Card Cottage settings must expose 100 front-image slots. Users should be able to upload, replace, delete, and drag-drop images into those slots with large, orderly controls.
- Card Cottage settings need enough width and an internal scroll area so all 100 image slots can be reached on iPad and desktop; do not let generic module dialog width caps squeeze or hide the slot grid.
- Card Cottage settings must also let the user set the deck's total card count. Keep this deck total separate from the 100 front-image slots; if the deck has more cards than filled image slots, repeat available images randomly.
...
- Word Camp must load complete server `wordProgress` before generating or persisting a new daily word plan. Do not let stale browser-local progress create today's plan before server progress arrives.
- Word Camp progress records should include per-record update metadata, and server merges must reject stale browser records that would roll newer mastery backward.
- Failed shared-state writes must be retained and retried after automatic server reconnection. A temporary iPad/network failure must not permanently switch the app into local-only mode or silently drop Word Camp progress.
- Failed shared-state patches must be persisted locally until the server confirms them. Do not keep pending progress/reward writes only in memory, because iPad refreshes or background tab eviction can otherwise drop completed Word Camp answers.
...
- Reward displays should show an explicit zero state. If global big stars are 0, show a subdued big-star icon with `0` instead of leaving the title-bar reward area visually empty.
- Card Cottage card reveal/spend must be a server-side atomic transaction. The client should request a card reveal by index and animate only after the server has confirmed the reveal, star deduction, and locked image.
